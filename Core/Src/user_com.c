...
uint8_t u1TxBuffer[DMA_TX_BUFFER_SIZE] __attribute__((section(".dma_buffer")));
uint8_t u1RxBuffer[DMA_RX_BUFFER_SIZE] __attribute__((section(".dma_buffer")));

uint8_t u2TxBuffer[DMA_TX_BUFFER_SIZE] __attribute__((section(".dma_buffer")));
uint8_t u2RxBuffer[DMA_RX_BUFFER_SIZE] __attribute__((section(".dma_buffer")));

uint8_t u4TxBuffer[DMA_TX_BUFFER_SIZE] __attribute__((section(".dma_buffer")));
uint8_t u4RxBuffer[DMA_RX_BUFFER_SIZE] __attribute__((section(".dma_buffer")));
...
struct uart_Stat_ST uart4_stat_ST;
struct usb_Stat_ST usb_stat_ST;

uint8_t usb_tx_usr_buf[DMA_TX_BUFFER_SIZE+2];
uint8_t rx_usb_line_buf[128];
...
void init_UART_COM(void)
{
	...
	UART_baudrate_set(&huart2,115200);
...
//
	...
	HAL_UART_Receive_DMA(&huart1,u2RxBuffer,DMA_RX_BUFFER_SIZE);
  //	__HAL_UART_ENABLE_IT(&huart2,UART_IT_IDLE);
  //	HAL_UART_Receive_DMA(&huart2,u2RxBuffer,DMA_RX_BUFFER_SIZE);
	__HAL_UART_ENABLE_IT(&huart4,UART_IT_IDLE);
	HAL_UART_Receive_DMA(&huart4,u4RxBuffer,DMA_RX_BUFFER_SIZE);
}
...
void printf_ESP(uint8_t color,const char *str_buf,...)
{
  char tx_bb[512];
  uint32_t len=0;
		...
		for(uint16_t i=0;i<q_len;i++)
		{
			...
			if (rx_dat == UART1_ETX)
			{
		  	...
		  	//CAN_str_trnsfer(line_buf,q_len);
				...
				if
				(
						(strncmp((char *)line_buf,"sden",4) == 0) ||
						(strncmp((char *)line_buf,"rqcf",4) == 0) ||
						(strncmp((char *)line_buf,"strt",4) == 0) ||
						(strncmp((char *)line_buf,"stop",4) == 0) ||
						(strncmp((char *)line_buf,"paus",4) == 0) ||
...
void printf_DBUG(uint8_t color,const char *str_buf,...)
{
  char tx_bb[512];
  uint32_t len=0;
